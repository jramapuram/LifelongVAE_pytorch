...
filenums = [re.match('fashion_hp_search(.*)__fid.csv', f, re.M|re.I).groups()[0] for f in filenames if 'fid' in f]
largest_experiment = np.max([int(f) for f in filenums]) + 1

# extract fids and elbos
fids = [('exp_{}'.format(i), pd.read_csv('experiments/fashion_hp_search{}__fid.csv'.format(i), header=None).fillna(np.inf).values) for i in filenums]
elbos = [('exp_{}'.format(i), pd.read_csv('experiments/fashion_hp_search{}__test_elbo.csv'.format(i), header=None).fillna(np.inf).values) for i in filenums]
consistencies = [('exp_{}'.format(i), pd.read_csv('experiments/fashion_hp_search{}__consistency.csv'.format(i), header=None).fillna(np.inf).values) for i in filenums]
...
for i in range(NUM_DISTRIBUTIONS):
    ...
    if i > 1:
        max_consistencies = sorted(consistencies, key=lambda t: t[1][i-1], reverse=True)[0:20]
