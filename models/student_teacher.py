...
class StudentTeacher(nn.Module):
    ...
    def _ewc(self, fisher_matrix, gamma=3.0):
        losses = []
    ...
    def fork(self):
        ...
        print("#teacher_samples: ", num_teacher_samples,
              " | #student_samples: ", max(self.config['batch_size'] - num_teacher_samples, 1))
    ...
    def generate_synthetic_sequential_samples(self, model, num_rows=8):
        ...
        num_discrete = model.reparameterizer.config['discrete_size']
        discrete_indices = np.asarray([[i]*num_rows for i in np.arange(num_discrete)])
        discrete_indices = discrete_indices.reshape(-1)
        with torch.no_grad():
            z_samples = Variable(torch.from_numpy(one_hot_np(num_discrete, discrete_indices)))
            ...
            if self.config['reparam_type'] == 'mixture':
                ...
                z_gauss = model.reparameterizer.gaussian.prior(
                    [num_rows*num_discrete, model.reparameterizer.gaussian.output_size]
                )
